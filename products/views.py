...
from products.models import Product, Category, Size, Color, ProductAttributes
...
def product_list(request):
    ...
    if query: ##搜尋可以透過搜尋商品名稱或描述
        products = products.filter(Q(name__icontains=query)|
                                   ...
                                   Q(tag__name__icontains=query)).distinct() 
    ###
...
def product_detail(request, slug):
    ...
    # 获取可用属性
    available_sizes = Size.objects.filter(productattributes__product=product).distinct()
    available_colors = Color.objects.filter(productattributes__product=product).distinct()
    ...
    return render(request, 'products/product_detail.html', {
        ...
        'category_products': category_products[:4]  # 添加回上下文
    })